...
void execute_one_inst(processor_t* p, int prompt, int print_regs)
{
  ...
  case 0x0: // opcode == 0x0 (SPECIAL)
    ...
    case 0x20: // funct == 0x20 (add)
      ...
      //printf("%d + %d = %d\n", (int) p->R[inst.rtype.rs], (int) p->R[inst.rtype.rt], p->R[inst.rtype.rd]);
      if ((int) p->R[inst.rtype.rs] < 0) {
        if ((int) p->R[inst.rtype.rd] > (int) p->R[inst.rtype.rs]) {
  ...
  case 0x9: // opcode == 0x9 (addiu)
    p->R[inst.itype.rt] = p->R[inst.itype.rs] + (uint32_t)((uint16_t)(inst.itype.imm));
    p->pc += 4;
    break;

  case 0xa: // opcode == 0xa (slti)
    if ((int) p->R[inst.itype.rt] < (uint32_t)((uint16_t)(inst.itype.imm))) {
      p->R[inst.itype.rt] = 1;
    } else {
      p->R[inst.itype.rt] = 0;
    }
  ...
  case 0xb: // opcode == 0xb (sltiu)
    if (p->R[inst.itype.rt] < (uint32_t)((uint16_t)(inst.itype.imm))) {
      p->R[inst.itype.rt] = 1;
    } else {
      p->R[inst.itype.rt] = 0;
    }
  ...
  case 0xf: // opcode == 0xf (lui)
    ...
    //printf("%d %d\n", p->R[inst.itype.imm], p->R[inst.itype.imm] << 16);
    p->pc += 4;
    break;
  ...
  case 0x20: // opcode == 0x20 (lb)
    p->R[inst.itype.rt] = (uint32_t)((uint16_t)load_mem(p->R[inst.itype.rs] + (uint32_t)((uint16_t)inst.itype.imm), SIZE_BYTE));
    p->pc += 4;
    break;

  case 0x23: // opcode == 0x23 (lw)
    p->R[inst.itype.rt] = load_mem(p->R[inst.itype.rs]+ (uint32_t)((uint16_t)inst.itype.imm), SIZE_WORD);
    p->pc += 4;
    break;

  case 0x24: // opcode == 0x24 (lbu)
    p->R[inst.itype.rt] = load_mem(p->R[inst.itype.rs] + (uint32_t)((uint16_t)inst.itype.imm), SIZE_BYTE);
    p->pc += 4;
    break;
  ...
  case 0x28: // opcode == 0x28 (sb)
    store_mem(p->R[inst.itype.rs] + (uint32_t)((uint16_t)inst.itype.imm), SIZE_BYTE, p->R[inst.itype.rt]);
    p->pc += 4;
    break;

  case 0x2b: // opcode == 0x2b (sw)
    store_mem(p->R[inst.itype.rs] + (uint32_t)((uint16_t)inst.itype.imm), SIZE_WORD, p->R[inst.itype.rt]);
    p->pc += 4;
    break;
